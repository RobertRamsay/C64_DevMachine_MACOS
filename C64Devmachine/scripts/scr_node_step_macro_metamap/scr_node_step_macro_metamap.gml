...
function scr_node_step_macro_metamap(_draw_x) {
    var header_h = 28;
    var pad      = 8;
    var line_h   = 18;
    ...
    if (mouse_check_button_pressed(mb_left) && !global.ui_click_consumed && !global.any_picker_open) {
        ...
        var _ms_x1 = _draw_x + 42;
        var _ms_x2 = _draw_x + 78;
        if (point_in_rectangle(mouse_x, mouse_y, _ms_x1, _mi_ly - 2, _ms_x2, _mi_ly + 12)) {
            while (array_length(instructions[0]) < 8) array_push(instructions[0], "");
            if (!is_real(instructions[0][6])) instructions[0][6] = 0;
            instructions[0][6] = (real(instructions[0][6]) == 0) ? 1 : 0;
        ...
        if (_src_mode == 1) {
            ...
            var _mv_x1 = _draw_x + 130;
            var _mv_x2 = _draw_x + width - 8;
            ...
            var _mi_mid = _draw_x + width - 44;
            var _mi_x2  = _draw_x + width - 8;
            var _mi_y1  = _mi_ly - 2;
            var _mi_y2  = _mi_ly + 14;
            var _cur    = (array_length(instructions[0]) > 2) ? real(instructions[0][2]) : 0;
            // Resolve map_count for clamp
            var _map_count = 0;
            var _tn = (array_length(instructions[0]) > 1) ? string(instructions[0][1]) : "";
    ...
    if (mouse_check_button_pressed(mb_left) && !global.ui_click_consumed && !global.any_picker_open) {
        ...
        if (point_in_rectangle(mouse_x, mouse_y, _zp_x1, _zp_ly - 2, _zp_x2, _zp_ly + 14)) {
            var _zp_cur = (array_length(instructions[0]) > 5) ? real(instructions[0][5]) : 0x50;
