/// @desc Handle LMB clicks for COND_IF nodes
function scr_node_step_cond_if(_draw_x) {
    ...
    // ── Row 1 right: CMP value (left half) + CMP VAR picker (right half) ───
    ...
    // Left: literal value — only clickable when no cmp var set
    ...
    if (!_has_cmp_var && point_in_rectangle(mouse_x, mouse_y, _cval_bx1, _cy + 4, _cval_bx2, _cy + 10)) {
        with (obj_workspace_manager) {
            ...
            if (global.use_hex_display) {
                ...
                while (string_length(_vh) < 2) _vh = "0" + _vh;
    ...
    // Right: CMP VAR picker — full width when a var is set
    ...
    if (point_in_rectangle(mouse_x, mouse_y, _cvar_bx1, _cy + 4, _cvar_bx2, _cy + 10)) {
        ...
        for (var _ki = 0; _ki < array_length(global.named_loc_meta); _ki++) {
            ...
            var _cme = global.named_loc_meta[_ki];
            var _cmsz = variable_struct_exists(_cme, "size") ? _cme.size : 1;
            if (_cmsz != 1) continue;
            array_push(label_picker_list, _cme.name);
        }
        exit;
    }
    _cy += _line_h;

    // ── Row 2: GOTO label — text modal ──────────────────────
    var _tgt_bx1 = _lx + 48;
    var _tgt_bx2 = _rx;
