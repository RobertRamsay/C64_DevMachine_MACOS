...
// Cache name normalization only; opcode_info remains live for every lookup.
normalized_names = {};
normalize_name = function(_name) {
    if (variable_struct_exists(normalized_names, _name)) return normalized_names[$ _name];
    ...
    if (array_length(variable_struct_get_names(normalized_names)) >= 512) normalized_names = {};
    normalized_names[$ _name] = _n;
    return _n;
};

get_cycles = function(_name) {
    var _n = normalize_name(_name);
