	...
	show_debug_message("[F6-A] set: c64u=" + string(trigger_c64u) + " build=" + string(trigger_build) + " ip=" + global.c64u_ip);
       ...
       var _zero_orgs = [];
        ...
        if (!ds_list_empty(global.node_chain)) {
            ...
            // deliberately excludes from the main traversal. Code Block nodes
            // (instructions[0][0] == "code_block") are also parsed for their
            // own labels/jumps, since a loop or an RTS written as hand-typed
            // text is just as real as one built from LABEL/JMP/RTS nodes.
            //
